...
function isAlphaNum(alphaNum) {
    ...
    return /^[a-zA-Z0-9]+$/.test(alphaNum) && !underscore
}

function emailCheck(email, username) {
    const validUsername = username.toLowerCase() === email.split('@')[0].toLowerCase()
    return /^\w+@\w+.com$/.test(email) && validUsername
}
...
function isAlpha(string) {
    ...
    return /^[a-zA-Z ]*$/.test(string) && !underscore
}
...
for (const entry of DATA) {
    ...
    if (!isAlphaNum(id) || !isAlphaNum(username) || !emailCheck(email, username) || !isNum(age) || !isAlpha(location)) {
